...
export async function GET(
  ...
  if (
    !assetPath ||
    !isSafeContentPath(assetPath) ||
    !/^pages\/history\/\d{3}-[^/]+\.(?:avif|gif|jpe?g|png|webp)$/i.test(assetPath)
  ) {
    return new Response("Not found", { status: 404 });
  }
  if (!(await getCanonicalBook(bookId))) return new Response("Not found", { status: 404 });
